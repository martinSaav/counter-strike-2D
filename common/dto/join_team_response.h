...
class JoinTeamResponse: public Message {
private:
    MessageType message_type = MessageType::JoinTeamResponse;
    bool success;

public:
    explicit JoinTeamResponse(bool success): success(std::move(success)) {}
    ...
    static JoinTeamResponse deserialize(const uint8_t* buffer, size_t size) {
        if (size < 3) {
        ...
        return JoinTeamResponse(success_deserialized);
    }
