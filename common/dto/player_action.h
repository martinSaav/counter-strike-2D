...
public:
    ...
    void serialize(uint8_t* buffer) const override {
        ...
        std::memcpy(buffer + 4, &pos_mouse_x, sizeof(pos_mouse_x));
        std::memcpy(buffer + 6, &pos_mouse_y, sizeof(pos_mouse_y));
    }
    ...
    static PlayerAction deserialize(const uint8_t* buffer, size_t size) {
        ...
        return PlayerAction(static_cast<Action>(action_deserialized), ntohs(pos_mouse_x), ntohs(pos_mouse_y));
    }
